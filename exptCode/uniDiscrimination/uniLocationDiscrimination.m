% Run 2 staircases seperately for A or V at 4 locations

% 1. split the session by A/V
% 2. randomize standard locations in a session
% 3. intermix 2 staircases (up/down) for each location

% V is the same stimulus used in localization task (splash of dots)
...
% standrd speaker/visual stimulus location (cm) in the main localization experiment
ExpInfo.n_speaker = 16;
ExpInfo.speaker_level_cm = linspace(-ExpInfo.speaker_cm, ExpInfo.speaker_cm, ExpInfo.n_speaker);
ExpInfo.sA_cm = ExpInfo.speaker_level_cm(ExpInfo.audLevel);
...
% trial info
ExpInfo.nLevel = numel(ExpInfo.audLevel); % for each standard location
...
for k = 1:ExpInfo.nLevel
    for i = 1:ExpInfo.n_trial_w_easy
        ExpInfo.condition(k, :, i) = randperm(ExpInfo.n_staircase, ExpInfo.n_staircase);
    end
end
...
ExpInfo.order = NaN(ExpInfo.nLevel, ExpInfo.n_staircase, ExpInfo.n_trial_w_easy);
for k = 1:ExpInfo.nLevel
    for i = 1:ExpInfo.n_staircase
        ExpInfo.order(k, i, :) = Shuffle(repmat([1, 2], 1, ExpInfo.n_trial_w_easy / 2));
    end
end
