function stimTextureMatrix = generateRippleStim(VSinfo,ExpInfo,ScreenInfo,windowPtr,i, height, noise_sd, stim_sd, wBack)
%% Gaussian Atlas
% Parameters
...
center_y = height / 2;
mean_luminance = 1.0;
targetLoc = [center_y,ScreenInfo.xmid + loc_pixel];

back_gaussian = mean_luminance * exp(-((x - center_x).^2 / (2 * noise_sd^2) + (y - center_y).^2 / (2 * back_sd_y^2)));
...
stim_gaussian = mean_luminance * exp(-((x - targetLoc(2)).^2 / (2 * stim_sd^2) + (y - targetLoc(1)).^2 / (2 * stim_sd_y^2)));
