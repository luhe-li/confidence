...
VSinfo.SD_yaxis            = 10; %SD of the blob in cm (vertical)
...
VSinfo.grayScreen                  = VSinfo.pblack * ones(ScreenInfo.xaxis,ScreenInfo.yaxis)*255;
VSinfo.gray_texture                = Screen('MakeTexture', windowPtr, VSinfo.grayScreen,[],[],[],2);
...
pscale                               = (1-VSinfo.pblack)/max(cloud_temp); % the max contrast of the blob adds the background contrast should <= 1
VSinfo.Cloud                         = reshape(cloud_temp,length(x),length(y));
...
ExpInfo.speakerLocCM = linspace(-ExpInfo.LRmostSpeakers2center, ExpInfo.LRmostSpeakers2center, ExpInfo.nLevel);
ExpInfo.speakerLocVA = linspace(-ExpInfo.LRmostVisualAngle, ExpInfo.LRmostVisualAngle, ExpInfo.nLevel);
...
ExpInfo.loc_deg = rad2deg(atan(Resp.loc_cm/ExpInfo.sittingDistance));
