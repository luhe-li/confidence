...
%%
    ...
    % confidence response
    SetMouse(x*2, yLoc*2, windowPtr);HideCursor(windowPtr);
    buttons = 0;
    WaitSecs(0.2);
    tic;
    while sum(buttons)==0
    ...
    tic;
    while sum(buttons)==0
        [caus_x,~,buttons] = GetMouse(windowPtr); HideCursor(windowPtr);
        caus_x = min(caus_x, ccSliderBound(2)); 
        caus_x = max(ccSliderBound(1),caus_x);
        Screen('DrawTexture',windowPtr, VSinfo.grey_texture,[],...
            [0,0,ScreenInfo.xaxis, ScreenInfo.yaxis]);
        Screen('DrawLine', windowPtr, [255 255 255],caus_x, yLoc+3, caus_x, yLoc-3, 1);
    ...
    Resp.RT3  = toc;
    HideCursor(windowPtr);
    Resp.estPcomm= caus_x - ccSliderBound(1);
