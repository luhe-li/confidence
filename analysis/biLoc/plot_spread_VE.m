...
a_rel = squeeze(mean(bi_resp(:,:,cue,reliability,:),5));
a_rel_std = std(squeeze(bi_resp(:,:,cue,reliability,:)),[],3);
...
aud_locs_mat = repmat(aud_locs,[4,1])';
index = aud_locs - aud_locs';
diff_vec = unique(index);
...
for i = 1:length(diff_vec)
    ...
    Eb(i) = errorbar(aud_locs_mat(index == diff_i),a_rel(index == diff_i),a_rel_std(index == diff_i),'-o' ...
        ,'Color',cMap_locR(1,i,:),'lineWidth', lw,'MarkerSize',15,...
         'MarkerEdgeColor',cMap_locR(1,i,:),'MarkerFaceColor',...
         cMap_locR(1,i,:));
    hold on
end
hold off
xlabel(sprintf('Audio stimulus location (deg)'),'FontSize',20);
ylabel(sprintf('Audio mean localization\nresponses (deg)'),'FontSize',20);
