...
out1FileName            = ['Adaptation_practice_sub',num2str(ExpInfo.subjID),...
...
ExpInfo.numTrials         = 2; %for each AV pair and response modality
ExpInfo.AVpairs_allComb   = combvec(1:4, 1:2); %1st row: AV loc, 2nd row: localization modality
ExpInfo.numAVpairs        = size(ExpInfo.AVpairs_allComb, 2); %8 possible pairs 
...
ExpInfo.localize_modality = localize_modality(ExpInfo.AVpairs_allComb(2,ExpInfo.AVpairs_order));
...
ExpInfo.numTrials_unity_perPair = ExpInfo.numTrials*0.5; %8 for each trial type, so 32 in total
...
VSinfo.arrangedLocs_deg   = VSinfo.Distance(ExpInfo.AVpairs_allComb(1,...
                            ExpInfo.AVpairs_order));
...
Adaptation_practice_data = {ExpInfo, ScreenInfo, VSinfo, AudInfo, Response};
save(out1FileName,'Adaptation_practice_data');
Screen('CloseAll');
