...
out1FileName            = ['Adaptation_incongruent_practice_sub',...
                            num2str(ExpInfo.subjID),'_session',...
                            num2str(ExpInfo.session)]; %create file name
...
VSinfo.spatialD    = [-15,15]; %-12 deg: V appears 12 degs to the left of A
VSinfo.temporalD   = [-250, 250]; %-250 ms: V precedes A for 250 ms
VSinfo.numFrames   = 6;
...
ExpInfo.AVpairs_allComb   = combvec(1:VSinfo.numLocs, 1:length(VSinfo.spatialD),...
                            1:length(VSinfo.temporalD), 1:length(localize_modality));
...
for i = 1:ExpInfo.numTrials
    ExpInfo.AVpairs_order = [ExpInfo.AVpairs_order, ...
        randperm(ExpInfo.numAVpairs,ExpInfo.numAVpairs)];
end
ExpInfo.localize_modality = localize_modality(ExpInfo.AVpairs_allComb(4,ExpInfo.AVpairs_order));

%find trials to insert unity judgment (unlike the bimodal-pre, we don't ask 
%subjects to report unity judgment on every single trial)
ExpInfo.numTrials_unity_perPair = ExpInfo.numTrials*1; %1 for each trial type, so 32 in total
ExpInfo.bool_unityReport        = zeros(1,ExpInfo.numTotalTrials);%1: insert unity judgment
for i = 1:VSinfo.numLocs
    idx = find(ExpInfo.AVpairs_order == i);
    selectedTrials = datasample(idx, ExpInfo.numTrials_unity_perPair,'Replace',false);
    ExpInfo.bool_unityReport(selectedTrials) = 1;
end

%Given the order of trial types, find the corresponding V and the A locations
VSinfo.arrangedLocs_deg = VSinfo.Distance(ExpInfo.AVpairs_allComb(1,ExpInfo.AVpairs_order))+...
                            VSinfo.spatialD(ExpInfo.AVpairs_allComb(2,ExpInfo.AVpairs_order));
VSinfo.arrangedLocs_cm  = round(tan(deg2rad(VSinfo.arrangedLocs_deg)).*...
                            ExpInfo.sittingDistance,2);
VSinfo.timing_relative  = VSinfo.temporalD(ExpInfo.AVpairs_allComb(3,ExpInfo.AVpairs_order));
...
for i = 1:ExpInfo.numTotalTrials 
    ...
    [Response.localization(i), Response.RT1(i), Response.unity(i),...
        Response.RT2(i)] = PresentMultisensoryStimuli_wTemporalD(i,ExpInfo,ScreenInfo,...
...
Adaptation_incongruent_practice_data = {ExpInfo, ScreenInfo, VSinfo, AudInfo, Response};
save(out1FileName,'Adaptation_incongruent_practice_data');
Screen('CloseAll');
