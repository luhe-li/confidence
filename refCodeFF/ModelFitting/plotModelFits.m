...
for i = 1:lenMT
    for j = 1:lenDS
        ...
        if bool_plot(1) == 1 
            ...
            %bootstrap
            [propC1_modelPred, propC1_param_btst_95CI, propC1_abs_modelPred,...
                propC1_abs_param_btst_95CI] = predictPropC1(R2);
            plotUnity(data.s_V, modelInfo, propC1_modelPred, ...
                propC1_param_btst_95CI, propC1_abs_modelPred, ...
                propC1_abs_param_btst_95CI, pC1, modelTypes{i}, stratG{j},...
                bool_save(1));
        end
...
switch M
    case 'HighPlasticityLongLasting' 
        [~, R1, R2] = nLL_HighPlasticityLongLasting(p(1),p(2),p(3),p(4),...
            p(5),p(6),p(7),p(8),p(9),p(10),p(11),p(12),p(13),p(14),p(15),...
            data,modelInfo);
        pC1 = round([p(9), p(10); p(11), p(12)],2);
    case 'HighPlasticityShortLasting'
        [~, R1, R2] = nLL_HighPlasticityShortLasting(p(1),p(2),p(3),...
            p(4),p(5),p(6),p(7),p(8),p(9),p(10),p(11),p(12),p(13),p(14),...
            data,modelInfo);
        pC1 = round([p(9), p(10); p(9), p(11)],2);   
    case 'NoPlasticity'
        [~, R1, R2] = nLL_NoPlasticity(p(1),p(2),p(3),p(4),p(5),p(6),...
            p(7),p(8),p(9),p(10),p(11),p(12),data,modelInfo);
        pC1 = round([p(9), p(9); p(9), p(9)],2);      
end

function [propC1_pred, propC1_param_btst_95CI, propC1_abs_pred, ...
    propC1_abs_param_btst_95CI] = predictPropC1(R2)
%output--------------------------------------------------------------------
%propC1_pred: the proportion of reporting C=1 as a function of spatial
%           discrepancy predicted by a given model
%propC1_param_btst_95CI: the 95% confidence interval (2 SD) on the 
...
%propC1_abs_param_btst_95CI: the 95% confidence interval (2 SD) on the 
...
idx_CI_lb                  = floor(n_param_btst*0.025);
idx_CI_ub                  = ceil(n_param_btst*0.975);
%initialize the proportion of reporting C=1
propC1_pred                = NaN(lenC, lenP, lenD);
propC1_abs_pred            = NaN(lenC, lenP, ceil(lenD/2));
propC1_param_btst_95CI     = NaN(lenC, lenP, lenD, 2); %last dimension:lb, ub
propC1_abs_param_btst_95CI = NaN(lenC, lenP, ceil(lenD/2), 2);
...
for i = 1:lenC %for each condition
    ...
    for j = 1:lenP    
        ...
        for l = 1:lenD
            ...
            propC1_param_btst_95CI(i,j,l,1) = propC1_param_btst_sort(idx_CI_lb);
            propC1_param_btst_95CI(i,j,l,2) = propC1_param_btst_sort(idx_CI_ub);
        end
        ...
        propC1_abs_param_btst_95CI(i,j,:,1) = propC1_abs_param_btst_sort(:,idx_CI_lb);
        propC1_abs_param_btst_95CI(i,j,:,2) = propC1_abs_param_btst_sort(:,idx_CI_ub);
    end
end
...
jitter         = [-0.6,0.6; 0.6,-0.6]; %[-0.6,0.6; 0.6,-0.6] or [0.6,-0.6; -0.6,0.6]
lw             = 2; %lineWidth
fs_lbls        = 20;
fs_lgds        = 18;
...
for i = 1:lenM
    ...
    for j = 1:lenC
        ...
        for k = 1:lenP
            ...
            plot(AV_offset+jitter(j,k), squeeze(VE_modelPred(j,k,i,:)),'-',...
            ...
            if bool_plt_95CI_paramSim
                ...
                patch([AV_offset+jitter(j,k), fliplr(AV_offset+jitter(j,k))], ...
        ...
        for k = 1:lenP    
            ...
            for n = 1:lenD
                plt(k) = errorbar(AV_offset(n)+jitter(j,k), meanVE(j,k,i,n),...
    ...
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.65, 0.55]);
...
for i = 1:lenM
    ...
    for j = 1:lenC
        ...
        for k = 1:lenP
            %plot model prediction
            plot(AV_abs_offset+jitter(j,k), squeeze(VE_abs_modelPred(j,k,i,:)),...
            ...
            if bool_plt_95CI_paramSim
                ...
                patch([AV_abs_offset+jitter(j,k), fliplr(AV_abs_offset+jitter(j,k))], ...
        ...
        for k = 1:lenP
            %plot behavioral data
            for n = 1:lenD_abs
                plt(k) = errorbar(AV_abs_offset(n)+jitter(j,k),...
                    meanVE_abs(j,k,i,n), 2.*seLocR_abs(j,k,i,n),...
                    '-o','MarkerSize',sqrt(numT_abs_AV(n)).*1.5, 'Color',cMap_VE{i,k},...
...
function plotUnity(s_V, model, propC1_modelPred, propC1_param_btst_95CI,...
    propC1_abs_modelPred, propC1_abs_param_btst_95CI, pC1, M, G, bool_save,...
    bool_plt_95CI_paramSim)
%if not specified, we will plot error bars on the model predictions
if nargin < 11; bool_plt_95CI_paramSim = 1;end
%global variables we need to use
...
jitter               = [-0.6,0.6; 0.6,-0.6]; %or [-0.6,0.6; 0.6,-0.6], [0.6,-0.6;-0.6,0.6]
lw                   = 2; %lineWidth
fs_lbls              = 20;
fs_lgds              = 18;
lgd_cond_phase       = {'Pre-learning', 'Post-learning'};
...
for i = 1:lenC
    ...
    for j = 1:lenP
        plot(AV_offset+jitter(i,j), squeeze(propC1_modelPred(i,j,:)),'Color',colorMap(j,:),...
            'lineWidth', lw, 'lineStyle','-'); hold on;
        if bool_plt_95CI_paramSim
            propC1_param_btst_95CI_lb = squeeze(propC1_param_btst_95CI(i,j,:,1));
            propC1_param_btst_95CI_ub = squeeze(propC1_param_btst_95CI(i,j,:,2));
            patch([AV_offset+jitter(i,j), fliplr(AV_offset+jitter(i,j))], ...
                [propC1_param_btst_95CI_lb',flipud(propC1_param_btst_95CI_ub)'],...
    ...
    for m = 1:length(AV_offset)
        %get the 2.5 and 97.5% percentiles
        [pC1_lenD_95CI_lb1, pC1_lenD_95CI_ub1] = ...
            bino_95CI(numT_AV(m), mean_pC1_lenD(i,1,m));
        [pC1_lenD_95CI_lb2, pC1_lenD_95CI_ub2] = ...
            bino_95CI(numT_AV(m), mean_pC1_lenD(i,2,m));

        plt_1 = errorbar(AV_offset(m)+jitter(i,1), mean_pC1_lenD(i,1,m),...
            mean_pC1_lenD(i,1,m) - pC1_lenD_95CI_lb1,... 
            pC1_lenD_95CI_ub1 - mean_pC1_lenD(i,1,m), '-o','MarkerSize',...
            sqrt(numT_AV(m)).*1.5,'Color', colorMap(1,:),'MarkerFaceColor',...
            colorMap(1,:),'MarkerEdgeColor',colorMap(1,:),'lineWidth',lw); hold on
        plt_2 = errorbar(AV_offset(m)+jitter(i,2),mean_pC1_lenD(i,2,m),...
            mean_pC1_lenD(i,2,m) -pC1_lenD_95CI_lb2, ...
            pC1_lenD_95CI_ub2 - mean_pC1_lenD(i,2,m),'-o','MarkerSize',...
            sqrt(numT_AV(m)).*1.5,'Color', colorMap(2,:),'MarkerFaceColor',...
            colorMap(2,:),'MarkerEdgeColor',colorMap(2,:),'lineWidth',lw); hold on
    end
...
for i = 1:lenC
    ...
    for j = 1:lenP
        ...
        plot(AV_abs_offset+jitter(i,j), squeeze(propC1_abs_modelPred(i,j,:)),...
            '-','lineWidth',lw,'Color',colorMap(j,:)); hold on;
        %plot parametric bootstrap
        if bool_plt_95CI_paramSim
            propC1_abs_param_btst_95CI_lb = squeeze(propC1_abs_param_btst_95CI(i,j,:,1));
            propC1_abs_param_btst_95CI_ub = squeeze(propC1_abs_param_btst_95CI(i,j,:,2));
            patch([AV_abs_offset+jitter(i,j), fliplr(AV_abs_offset+jitter(i,j))], ...
                [propC1_abs_param_btst_95CI_lb',flipud(propC1_abs_param_btst_95CI_ub)'],...
                colorMap(j,:),'FaceAlpha', 0.2, 'EdgeColor',...
                colorMap(j,:),'EdgeAlpha', 0.2,'lineWidth',2); hold on;
        end
    end
    ...
    for j = 1:lenP
        ...
        for m = 1:lenD_abs
            %get the 2.5 and 97.5% percentiles
            [pC1_abs_lenD_95CI_lb, pC1_abs_lenD_95CI_ub] = ...
                bino_95CI(numT_abs_AV(m), meanPC1_abs_spatialD(i,j,m));
      
            plt(j) = errorbar(AV_abs_offset(m)+jitter(i,j), ...
                meanPC1_abs_spatialD(i,j,m), meanPC1_abs_spatialD(i,j,m)- ...
                pC1_abs_lenD_95CI_lb,pC1_abs_lenD_95CI_ub-meanPC1_abs_spatialD(i,j,m),'-o',...
                'MarkerSize',sqrt(numT_abs_AV(m)).*1.5,'Color',colorMap(j,:),...
...
function [p_lb, p_ub] = bino_95CI(N, P)
N_vec      = 0:N;
bino_p     = binopdf(N_vec, N, P);
bino_cum_p = cumsum(bino_p);
p_lb       = N_vec(find(bino_cum_p > 0.025, 1))/N;
p_ub       = N_vec(find(bino_cum_p > 0.975, 1))/N;
