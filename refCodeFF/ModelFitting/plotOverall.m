...
%'3-PW'  : HighPlasticityLongLasting     MA-MAP          /
%'4-SW'  : NoPlasticity                  MA-measurements /
%'5-HL'  : NoPlasticity                  MS-posteriorC1  /
%'6-YZ'  : HighPlasticityLongLasting     MS-posteriorC1  /
%'8-NH'  : NoPlasticity                  MA-MAP          /
%'9-ZZ'  : HighPlasticityLongLasting     MA-MAP          /
%'11-BB' : HighPlasticityLongLasting     MS-posteriorC1  /
%'12-ZY' : HighPlasticityLongLasting     MS-posteriorC1  /
%'13-MR' : HighPlasticityShortLasting    MA-MAP          /
%'18-ZL' : HighPlasticityShortLasting    MA-MAP          /
%'19-RE' : HighPlasticityLongLasting     MS-posteriorC1  /
%'20-MM' : HighPlasticityLongLasting     MA-measurements /
%'21-LL' : HighPlasticityShortLasting    MA-MAP          /
%'22-CS' : HighPlasticityLongLasting     MA-MAP          /
%'23-JH' : HighPlasticityShortLasting    MA-MAP          /
%'24-MD' : HighPlasticityLongLasting     MA-MAP          /
%'25-HHL': NoPlasticity                  MA-MAP          /
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
modelTypes_dict = {'HighPlasticityShortLasting', ...
                   'HighPlasticityLongLasting',...
                   'NoPlasticity'};
%decision strategy
strategy_dict   = {'strategyLocResp_MA_strategyUnity_posteriorC1',...
                   'strategyLocResp_MA_strategyUnity_measurements',...
                   'strategyLocResp_MA_strategyUnity_MAP',...
                   'strategyLocResp_MS_strategyUnity_posteriorC1',...
                   'strategyLocResp_MS_strategyUnity_measurements'};

%create popout window
prompt     = {'Subject ID (0: all subjects):',['Model of pCommon ',...
    '(1:High Plasticity Short Lasting, 2: High Plasticity LongLasting,'...
    '3: No Plasticity, 0:all models)'], ['Model of strategy ',...
...
idx_mat    = {[9,10;9,11],[9,10;11,12],[9,9;9,9]}; %order is the same as modelTypes
...
for i = 1:nS
    ...
    for j = 1:lenMT
        jj = find(contains(modelTypes_dict,modelTypes{j}));
        for k = 1:lenDS
            pC1_all(i,j,k,1,:) = estimatedP{i}{j,k}(idx_mat{jj}(1,:)); %cong condition
            pC1_all(i,j,k,2,:) = estimatedP{i}{j,k}(idx_mat{jj}(2,:)); %incong condition
        end
    end
...
if str2double(answer(2)) == 0  && str2double(answer(3)) == 0
    ...
    numParams       = [13,14,14,13,14; 14,15,15,14,15; 11,12,12,11,12]; 
...
if str2double(answer(1)) == 0 && bool_plot(6)
    nBtst  = 120; %we bootstrap 120 times
    CI_bds = [0.025, 0.975];
    ...
    for i = 1:nS
        ...
        switch Model_name
            case 'HighPlasticityShortLasting'; idx = idx_mat{1};
            case 'HighPlasticityLongLasting';  idx = idx_mat{2};
            case 'NoPlasticity';               idx = idx_mat{3};
        end
        ...
        for j = 1:lenP
            ...
            pC1_btst_lb(i,:,j)   = pC1_pre_post_btst{j}(nBtst*CI_bds(1),:);
            pC1_btst_ub(i,:,j)   = pC1_pre_post_btst{j}(nBtst*CI_bds(2),:);
        end
...
grp  = {{'MR','LL','ZZ','MM','CS','RE'},{'PW','ZL','JH','YZ','BB','ZY','MD'}};
choice_cmap = 'rainbow';
[pC1_bestM_grp, diff_pC1_bestM_grp, pC1_btst_lb_grp, pC1_btst_ub_grp,...
    diff_pC1_btst_lb_grp, diff_pC1_btst_ub_grp] = deal(cell(1, length(grp)));
if str2double(answer(1)) == 0 && bool_plot(7)
    for i = 1:length(grp)
        %find the corresponding indices for those selected participants
        indices                 = arrayfun(@(idx) find(strcmp(grp{i}{idx},...
                                    subjIs) == 1,1), 1:length(grp{i}));
        pC1_bestM_grp{i}        = pC1_bestM(indices,:,:);
        diff_pC1_bestM_grp{i}   = diff_pC1_bestM(indices, :);
        pC1_btst_lb_grp{i}      = pC1_btst_lb(indices,:,:);
        pC1_btst_ub_grp{i}      = pC1_btst_ub(indices,:,:);
        diff_pC1_btst_lb_grp{i} = diff_pC1_btst_lb(indices,:,:);
        diff_pC1_btst_ub_grp{i} = diff_pC1_btst_ub(indices,:,:);
        plot_bestfitPC1(pC1_bestM_grp{i}, pC1_btst_lb_grp{i}, pC1_btst_ub_grp{i}, ...
            diff_pC1_bestM_grp{i}, diff_pC1_btst_lb_grp{i}, diff_pC1_btst_ub_grp{i}, ...
            choice_cmap, grp{i}, bool_save(7), indices)
    end
    %display mean (absolute) difference
    all_pC1_bestM_cong = [diff_pC1_bestM_grp{1}(:,1); diff_pC1_bestM_grp{2}(:,1)];
    all_pC1_bestM_incong = [diff_pC1_bestM_grp{1}(:,2); diff_pC1_bestM_grp{2}(:,2)];
    mean_abs_diff_pC1_bestM_cong = mean(abs(all_pC1_bestM_cong));
    mean_abs_diff_pC1_bestM_incong = mean(abs(all_pC1_bestM_incong));
    sem_abs_diff_pC1_bestM_cong = std(abs(all_pC1_bestM_cong))/sqrt(length(all_pC1_bestM_cong));
    sem_abs_diff_pC1_bestM_incong = std(abs(all_pC1_bestM_incong))/sqrt(length(all_pC1_bestM_cong));  
    fprintf('Congruent: mean absolute difference = %.3f, SEM = %.3f.\n', ...
        mean_abs_diff_pC1_bestM_cong, sem_abs_diff_pC1_bestM_cong);
    fprintf('Congruent: mean absolute difference = %.3f, SEM = %.3f.\n', ...
        mean_abs_diff_pC1_bestM_incong, sem_abs_diff_pC1_bestM_incong);
    
    %make a scatter plot
    figure
    cMAP  = {[125,203,151;  89,191,182;  78,135,163;   79,79,119;    93,46,88;...
           148,48,81]./255,[195,56,79;  243,115,83;  245,159,77;  249,204,84;...
         237,225,121; 187,216,123;  210,105,30; 100,100,100; 200,200,200;...
         110,200,250;  138,82,192]./255};
    marker_grp = {'square','^'};

    for k = 1:length(grp)
        for j = 1:length(grp{k})
            scatter(diff_pC1_bestM_grp{k}(j,1), diff_pC1_bestM_grp{k}(j,2),...
                100, cMAP{k}(j,:), marker_grp{k},'MarkerFaceColor',cMAP{k}(j,:),...
                'MarkerFaceAlpha', 0.7, 'lineWidth',2); hold on
            errorbar(diff_pC1_bestM_grp{k}(j,1), diff_pC1_bestM_grp{k}(j,2),...
                diff_pC1_bestM_grp{k}(j,2) - diff_pC1_btst_lb_grp{k}(j,2),...
                diff_pC1_btst_ub_grp{k}(j,2) - diff_pC1_bestM_grp{k}(j,2),...
                diff_pC1_bestM_grp{k}(j,1) - diff_pC1_btst_lb_grp{k}(j,1),...
                diff_pC1_btst_ub_grp{k}(j,1) - diff_pC1_bestM_grp{k}(j,1),'-',...
                'MarkerFaceColor',0.5.*ones(1,3),'MarkerEdgeColor',0.5.*ones(1,3),...
                'lineWidth',1,'Color',0.5.*ones(1,3)); hold on
        end
    end
    xlabel(sprintf('Change of the common-cause prior \nafter a congruent learning phase'));
    ylabel(sprintf('Change of the common-cause prior \nafter an incongruent learning phase'));
    xlim([-0.5, 0.7]); ylim([-0.5, 0.7]); axis square; box off;
    set(gca,'FontSize',18); grid on
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.4, 0.65]);
    set(gcf,'PaperUnits','centimeters','PaperSize',[25 25]);
    %saveas(gcf,'Delta_pC1_cong_vs_incong.pdf');
end
