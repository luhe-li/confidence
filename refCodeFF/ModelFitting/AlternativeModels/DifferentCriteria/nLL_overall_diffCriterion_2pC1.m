function [nLL, R1, R2] = nLL_overall_diffCriterion_2pC1(a_A, b_A, sigma_A, sigma_V,...
    sigma_AV_A, sigma_AV_V, c_pre_cong, c_post_cong, c_pre_incong, c_post_incong,...
    pC1_cong, pC1_incong, lapse_matching, lapse_unity_cong, lapse_unity_incong, data, model)
...
criterion   = [c_pre_cong, c_post_cong; c_pre_incong, c_post_incong];
...
for i = 1:length(model.cond) 
    ...
    for j = 1:length(model.phase) 
        ...
        [nLL_bimodal(i,j),R2{i,j}] = calculateNLL_bimodal(pC1(i,j), data.s_A,...
            data.s_V, a_A, b_A, sigma_AV_A, sigma_AV_V, criterion(i,j), CI, mu_P,...
...
function [nLL_bimodal,R] = calculateNLL_bimodal(pC1, s_A, s_V, a_A, b_A, ...
    sigma_AV_A, sigma_AV_V, c, CI, mu_P, lapse, D_unity, D_locResp, ...
    N, sigma_r, model)
nLL_bimodal = 0;
...
for p = 1:length(s_A_prime)   %for each AV pair with s_A' = s_A_prime(p)
    ...
    for q = 1:length(s_V_prime) %for each AV pair with s_V' = s_V_prime(q)
        ...
        if strcmp(model.strategy_unity,'MAP')
            I_C1_resp = squeeze(double(abs(MAP_MA(1,:,:) - MAP_MA(2,:,:)) < c));  
        elseif strcmp(model.strategy_unity, 'measurements')
            I_C1_resp = double(abs(GRID_X1 - GRID_X2) < c);  
        else %'posteriorC1'
            I_C1_resp = double((Post_C1 > c));
        end
