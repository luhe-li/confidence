function [nLL, R1, R2] = nLL_overall_HA(a_A, b_A, sigma_A, sigma_V, sigma_AV_A,...
...
for i = 1:length(model.cond) 
    ...
    for j = 1:length(model.phase) 
        ...
        [nLL_bimodal(i,j),R2{i,j}] = calculateNLL_bimodal(pC1(i,j), data.s_A,...
            data.s_V, a_A, b_A, sigma_AV_A, sigma_AV_V, epsilon, weight, CI, mu_P,...
...
function [nLL_bimodal,R] = calculateNLL_bimodal(pC1, s_A, s_V, a_A, b_A, ...
    sigma_AV_A, sigma_AV_V, epsilon, w, CI, mu_P, lapse, D_unity, D_locResp, ...
    N, sigma_r, model)
nLL_bimodal = 0;
...
for p = 1:length(s_A_prime)   %for each AV pair with s_A' = s_A_prime(p)
    ...
    for q = 1:length(s_V_prime) %for each AV pair with s_V' = s_V_prime(q)
        ...
        MAP_MA(1,:,:)  = shat_C1.*Post_C1 + squeeze(shat_C2(1,:,:)).*Post_C2; %MAP_MA_A
        MAP_MA(2,:,:)  = shat_C1.*Post_C1 + squeeze(shat_C2(2,:,:)).*Post_C2; %MAP_MA_V
        ...
        for k = 1:N
            ...
            if D_unity(p,q,k)==1 %same source
                ...
                p_r_given_MAP = norm_dst(locResp_A_V(k),...
                    squeeze(MAP_MA(1,:,:).*w + MAP_MA(2,:,:).*(1-w)),sigma_r,1e-20);
        ...
        R.MAP(p,q,:,:,:) = MAP_MA;
        idx_selected = squeeze(abs(MAP_MA(1,:,:) - MAP_MA(2,:,:)) < epsilon);
        R.MAP(p,q,:,idx_selected) = repmat(MAP_MA(1,idx_selected).*w + ...
            MAP_MA(2,idx_selected).*(1-w),[2,1,1]); %weighted sum
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end
end
