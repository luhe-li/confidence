...
clear all; close all; clc; parpool(2); %2 core; 2.5 hours
subjN = 15;
subjI = 'AD';
...
model.modality       = ['A','V'];
model.strategy_MAP   = 'HA'; %Heuristic averaging
model.strategy_unity = 'MAP'; %compare the distance btw shat_A and shat_V
...
%p(15)      : weight assigned to shat_A
%--------------------------------------------------------------------------
nLogL     = @(p) nLL_overall_HA(p(1),p(2),p(3),p(4),p(5),p(6),p(7),p(8),p(9),...
                    p(10),p(11),p(12),p(13),p(14),p(15),data,model);
model.lb  = [0.5,-20, 1e-1, 1e-2,1e-1, 1e-2, 0.1,  1e-2, 1e-2,1e-2, 1e-2, 1e-4, 1e-4, 1e-4,   0];
model.ub  = [3.5, 10,   15,    5,  15,   10,  30,  0.99, 0.99,0.99, 0.99, 0.06, 0.06, 0.06,   1];
model.plb = [1.5, -9,    1,  0.1,   5,    2,   5,   0.1,  0.1, 0.1,  0.1, 1e-2, 1e-2, 1e-2, 0.1];
model.pub = [  2,  0,    5,    2,  10,    5,  10,   0.5,  0.5, 0.5,  0.5, 0.03, 0.03, 0.03, 0.9];
model.numRuns     = 20;
model.numSect_bds = 4;
