...
clear all; close all; clc; parpool(3); %3 cores, 3 hours
subjN = 20;
subjI = 'MM';
...
%p(16)-p(18): lapse rate for the matching task and the unity judgments for
%               the congruent and incongruent condition
%--------------------------------------------------------------------------
nLogL     = @(p) nLL_overall_diffSigma(p(1),p(2),p(3),p(4),p(5),p(6),p(7),...
                p(8),p(9),p(10),p(11),p(12),p(13),p(14),p(15),p(16),p(17),p(18),data,model);
model.lb  = [0.5,-20, 1e-1, 1e-2,1e-1,1e-1,1e-1, 1e-2, 1e-2, 1e-2, 0.1, 1e-2, 1e-2,1e-2, 1e-2, 1e-4, 1e-4, 1e-4];
model.ub  = [3.5, 10,   15,    5,  15,  15,  15,    5,    5,    5,  20, 0.99, 0.99,0.99, 0.99, 0.06, 0.06, 0.06];
model.plb = [1.5, -9,    1,  0.1,   5,   5,   5,    1,    1,    1,   5,  0.1,  0.1, 0.1,  0.1, 1e-2, 1e-2, 1e-2];
model.pub = [  2,  0,    5,    2,  10,  10,  10,    3,    3,    3,  10,  0.5,  0.5, 0.5,  0.5, 0.03, 0.03, 0.03];
