function [nLL, R1, R2] = nLL_overall_samePC1cond_diffSigma(a_A, b_A, sigma_A, ...
    sigma_V, sigma_AV_A_pre,sigma_AV_A_post_cong, sigma_AV_A_post_incong,...
    sigma_AV_V_pre, sigma_AV_V_post_cong, sigma_AV_V_post_incong, epsilon,...
    pC1_cong, pC1_incong, lapse_matching, lapse_unity_cong, lapse_unity_incong, ...
    data, model)
...
pC1         = [pC1_cong, pC1_cong; pC1_incong, pC1_incong];
...
for i = 1:length(model.cond) 
    ...
    for j = 1:length(model.phase) 
        %sigma changes after the learning phase
        if j == 1;sigma_AV_A = sigma_AV_A_pre; sigma_AV_V = sigma_AV_V_pre;
        else
            if i == 1; sigma_AV_A = sigma_AV_A_post_cong;sigma_AV_V = sigma_AV_V_post_cong;
            else; sigma_AV_A = sigma_AV_A_post_incong;sigma_AV_V = sigma_AV_V_post_incong;
            end
        end
