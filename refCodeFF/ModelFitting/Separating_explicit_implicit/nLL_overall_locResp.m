function [nLL, R1, R2] = nLL_overall_locResp(a_A, b_A, sigma_A, sigma_V,...
    sigma_AV_A, sigma_AV_V, pC1_pre_cong, pC1_post_cong, pC1_pre_incong, ...
    pC1_post_incong, lapse_matching, data, model)
...
for i = 1:length(model.cond) 
    for j = 1:length(model.phase) 
        ...
        [nLL_bimodal(i,j),R2{i,j}] = calculateNLL_bimodal(pC1(i,j), data.s_A,...
            data.s_V, a_A, b_A, sigma_AV_A, sigma_AV_V, CI, mu_P,...
            Dij_locResp, data.numUnityTrialsPerLoc, data.sigma_r, model);
    end
end
...
function [nLL_bimodal,R] = calculateNLL_bimodal(pC1, s_A, s_V, a_A, b_A, ...
    sigma_AV_A, sigma_AV_V, CI, mu_P, D_locResp, N, sigma_r, model)
nLL_bimodal = 0;
...
for p = 1:length(s_A_prime)   %for each AV pair with s_A' = s_A_prime(p)
    ...
    for q = 1:length(s_V_prime) %for each AV pair with s_V' = s_V_prime(q)
        ...
        if strcmp(model.strategy_MAP,'MA') %Model averaging
            ...
            for k = 1:N
                idx = fix((k-1)/(N/2))+1;
                p_r_given_MAP_C1 = norm_dst(locResp_A_V(k),shat_C1,sigma_r,1e-20);
                p_r_given_MAP_C2 = norm_dst(locResp_A_V(k),...
                                    squeeze(shat_C2(idx,:,:)), sigma_r,1e-20);
                p_r_given_MAP    = p_r_given_MAP_C1;
                p_r_given_MAP(Post_C1<=0.5) = p_r_given_MAP_C2(Post_C1<=0.5);
                nLL_bimodal      = nLL_bimodal - log(sum(sum(p_r_given_MAP.*...
                                    p_mAmV_given_sAsV)));
            end 
        end
