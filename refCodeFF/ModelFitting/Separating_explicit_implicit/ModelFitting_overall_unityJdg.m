...
clear all; close all; clc; parpool(1);
subjN = 20;
subjI = 'MM';
...
%--------------------------------------------------------------------------
nLogL     = @(p) nLL_overall_unityJdg(p(1),p(2),p(3),p(4),p(5),p(6),...
            p(7),p(8),p(9),p(10),p(11),p(12),p(13),p(14),data,model);
model.lb  = [0.5,-20, 1e-1, 1e-3,1e-1, 1e-3, 0.1, 1e-2, 1e-2,1e-2, 1e-2, 1e-4, 1e-4, 1e-4];
model.ub  = [3.5, 10,   15,    5,  15,   10,  30, 0.99, 0.99,0.99, 0.99, 0.06, 0.06, 0.06];
...
save(['ModelFitting_updatePrior_fullModel_strategyUnity_', ...
    model.strategy_unity, '_sub', num2str(subjN), '_', datestr(datetime('now')),...
    '.mat'],'ModelFitting');
