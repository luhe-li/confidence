...
%p(7)-p(8)  : pC1_pre_cong, pC1_post_cong, pCommon for the pre- and the
%               post-association phase
%p(9)-p(10) : pC1_pre_incong, pC1_post_incong, pCommon for the pre- and the
%               post-dissociation phase
%p(11): lapse rate for the matching task
%--------------------------------------------------------------------------
nLogL     = @(p) nLL_overall_locResp(p(1),p(2),p(3),p(4),p(5),p(6),...
            p(7),p(8),p(9),p(10),p(11),data,model);
model.lb  = [0.5,-20, 1e-1, 1e-3,1e-1, 1e-3, 1e-2, 1e-2,1e-2, 1e-2, 1e-4];
model.ub  = [3.5, 10,   15,    5,  15,   10, 0.99, 0.99,0.99, 0.99, 0.06];
model.plb = [1.5, -9,    1,  0.1,   5,    2,  0.1,  0.1, 0.1,  0.1, 1e-2];
model.pub = [  2,  0,    5,    2,  10,    5,  0.5,  0.5, 0.5,  0.5, 0.03];
model.numRuns     = 20;
model.numSect_bds = 4;
