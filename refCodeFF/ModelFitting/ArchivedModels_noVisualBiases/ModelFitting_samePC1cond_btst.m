...
%p(8)-p(9)  : pC1_cong, pC1_incong
%p(10)-p(12): lapse rate for the matching task and the unity judgments for
%               the congruent and incongruent condition
%--------------------------------------------------------------------------
model.lb  = [0.5,-20, 1e-1, 1e-3,1e-1, 1e-1, 0.1, 1e-2, 1e-2, 1e-4, 1e-4, 1e-4];
model.ub  = [  3,  5,   15,    5,  15,   10,  30, 0.99, 0.99, 0.06, 0.06, 0.06];
model.plb = [1.5, -9,    1,  0.1,   5,    2,   5,  0.1,  0.1, 1e-2, 1e-2, 1e-2];
model.pub = [  2,  0,    5,    2,  10,    5,  10,  0.5,  0.5, 0.03, 0.03, 0.03];
...
parfor n = 1:model.numBtst
    ...
    nLogL     = @(p) nLL_overall_samePC1cond(p(1),p(2),p(3),p(4),p(5),p(6),...
                p(7),p(8),p(9),p(10),p(11),p(12),D_btst{n},model);
...
save(['ModelFitting_updatePrior_samePC1cond_strategyMAP_', model.strategy_MAP, ...
