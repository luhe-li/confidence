function [nLL, R1, R2] = nLL_overall(a_A, b_A, sigma_A, sigma_V, sigma_AV_A,...
    sigma_AV_V, epsilon, pC1_pre_cong, pC1_post_cong, pC1_pre_incong, ...
    pC1_post_incong, lapse_matching, lapse_unity_cong,lapse_unity_incong, ...
    data, model)
...
for p = 1:length(s_A_prime)   %for each AV pair with s_A' = s_A_prime(p)
    ...
    for q = 1:length(s_V_prime) %for each AV pair with s_V' = s_V_prime(q)
        ...
        else %model selection & measurements-based heuristics
            for m = 1:length(model.modality)
                R.MAP(p,q,m,:,:) = shat_C1;
...
% %auditory location estimate
% figure; imagesc(x1_grid, x2_grid, squeeze(MAP_MA(1,:,:)));colormap(winter);colorbar; 
% xlabel('$m_{AV_l,A}''$','interpreter','latex');ylabel('$m_{AV_l,V}''$','interpreter','latex');
% title('$\hat{s}_{AV_l,A}''$','interpreter','latex');
% xticks([ceil(x1_grid(1)), round(s_A_prime(p)), floor(x1_grid(end))]); 
% yticks([ceil(x2_grid(1)), round(s_V_prime(q)), floor(x2_grid(end))]); set(gca,'FontSize',20);
% set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.35, 0.55]); 
% caxis([floor(min(min(squeeze(MAP_MA(1,:,:))))), ceil(max(max(squeeze(MAP_MA(1,:,:)))))]);
% set(gcf,'PaperUnits','centimeters','PaperSize',[25 25]);%saveas(gcf, 'AuditoryLocEstimate_byMA','pdf');
% 
% %visual location estimate
% figure; imagesc(x1_grid, x2_grid, squeeze(MAP_MA(2,:,:)));colormap(winter);colorbar; 
% xlabel('$m_{AV_l,A}''$','interpreter','latex');ylabel('$m_{AV_l,V}''$','interpreter','latex');
% title('$\hat{s}_{AV_l,V}''$','interpreter','latex');
% xticks([ceil(x1_grid(1)), round(s_A_prime(p)), floor(x1_grid(end))]); 
% yticks([ceil(x2_grid(1)), round(s_V_prime(q)), floor(x2_grid(end))]); set(gca,'FontSize',20);
% set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.35, 0.55]);
% caxis([floor(min(min(squeeze(MAP_MA(1,:,:))))), ceil(max(max(squeeze(MAP_MA(1,:,:)))))]);
% set(gcf,'PaperUnits','centimeters','PaperSize',[25 25]);%saveas(gcf, 'VisualLocEstimate_byMA','pdf');
% 
% %difference
% figure; imagesc(x1_grid, x2_grid, squeeze(MAP_MA(2,:,:) - MAP_MA(1,:,:)));colormap(winter);colorbar; 
% xlabel('$m_{AV_l,A}''$','interpreter','latex');ylabel('$m_{AV_l,V}''$','interpreter','latex');
% title('$\hat{s}_{AV_l,V}'' - \hat{s}_{AV_l,A}''$','interpreter','latex');
% xticks([ceil(x1_grid(1)), round(s_A_prime(p)), floor(x1_grid(end))]); 
% yticks([ceil(x2_grid(1)), round(s_V_prime(q)), floor(x2_grid(end))]); set(gca,'FontSize',20);
% set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.35, 0.55]);
% caxis([floor(min(min(squeeze(MAP_MA(1,:,:))))), ceil(max(max(squeeze(MAP_MA(1,:,:)))))]);
% set(gcf,'PaperUnits','centimeters','PaperSize',[25 25]);%saveas(gcf, 'Diff_LocEstimates_byMA','pdf');
%  
% %intended 'common-cause' responses 
% figure; imagesc(x1_grid, x2_grid, I_C1_resp);colormap(gray);colorbar; 
% xlabel('$m_{AV_l,A}''$','interpreter','latex');ylabel('$m_{AV_l,V}''$','interpreter','latex');
% title('$I_{C=1}=1$','interpreter','latex');
% xticks([ceil(x1_grid(1)), round(s_A_prime(p)), floor(x1_grid(end))]); 
% yticks([ceil(x2_grid(1)), round(s_V_prime(q)), floor(x2_grid(end))]); set(gca,'FontSize',20);
% set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.35, 0.55]); caxis([0,1]);
% set(gcf,'PaperUnits','centimeters','PaperSize',[25 25]); %saveas(gcf, 'IntendedCommonCauseJdg','pdf');
% 
% %actual 'common-cause' response
% figure; imagesc(x1_grid, x2_grid, P_C1_resp);colormap(gray);colorbar; 
% xlabel('$m_{AV_l,A}''$','interpreter','latex');ylabel('$m_{AV_l,V}''$','interpreter','latex');
% title('$P(I_{C=1}=1|m_{AV_l,A}'', m_{AV_l,V}'')$','interpreter','latex');
% xticks([ceil(x1_grid(1)), round(s_A_prime(p)), floor(x1_grid(end))]); 
% yticks([ceil(x2_grid(1)), round(s_V_prime(q)), floor(x2_grid(end))]); set(gca,'FontSize',20);
% set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.35, 0.55]); caxis([0,1]);
% set(gcf,'PaperUnits','centimeters','PaperSize',[25 25]); %saveas(gcf, 'ActualCommonCauseJdg','pdf');
% 
