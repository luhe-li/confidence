...
subjN = 23;   %   13,   18,   21,  23
subjI = 'JH'; % 'MR', 'ZL', 'LL', 'JH'
...
%free parameters for the restricted model
%--------------------------------------------------------------------------
...
%p(9)       : pC1_pre
%p(10)-p(11): pC1_post_cong, pC1_post_incong, pCommon for the pre- and the
...
%--------------------------------------------------------------------------
model.lb  = [0.5,-20,   1, 1e-1, 1e-3,1e-1, 1e-3, 0.1, 1e-2,1e-2, 1e-2, 1e-4, 1e-4, 1e-4];
model.ub  = [3.5,  5,  30,   15,    5,  15,   10,  10, 0.99,0.99, 0.99, 0.06, 0.06, 0.06];
model.plb = [  1, -5,   5,    1,  0.1,   5,    2,   1,  0.1, 0.1,  0.1, 1e-2, 1e-2, 1e-2];
model.pub = [2.5,  0,  20,    5,    2,  10,    5,   5,  0.5, 0.5,  0.5, 0.03, 0.03, 0.03];
...
parfor n = 1:model.numBtst
    ...
    nLogL   = @(p) nLL_HighPlasticityShortLasting(p(1),p(2),p(3),p(4),...
...
save(['ModelFitting_updatePrior_HighPlasticityShortLasting_strategyLocResp_',...
