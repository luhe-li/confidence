function [nLL, R1, R2] = nLL_overall_wLearningPhase_diffSigma(a_A, b_A,...
    sigma_A, sigma_V, sigma_AV_A_prepost, sigma_AV_A_cong,sigma_AV_A_incong, ...
    sigma_AV_V_prepost, sigma_AV_V_cong, sigma_AV_V_incong, epsilon, pC1_pre_cong,...
    pC1_post_cong, pC1_pre_incong, pC1_post_incong, lapse_matching,...
    lapse_unity_cong, lapse_unity_incong, data, model)
...
%                    3rd dim, loc A index
%                    4th dim, loc V index
%data.bimodal_locResp: 5th dim, response modality (1: A; 2: V)
%                      6th dim, trial number
...
nLL_learning= NaN(1,length(model.cond));
for i = 1:length(model.cond) 
    lapse_unity_i = lapse_unity(i);
    sigma_AV_A    = sigma_AV_A_prepost;
    sigma_AV_V    = sigma_AV_V_prepost;
    CI_prepost    = calcCI(sigma_AV_A,sigma_AV_V,sigma_P);
    for j = 1:length(model.phase) 
        Dij_unity        = squeeze(data.bimodal_unity(i,j,:,:,:));
        Dij_locResp      = squeeze(data.bimodal_locResp(i,j,:,:,:,:));
        [nLL_bimodal(i,j),R2{i,j}] = calculateNLL_bimodal(pC1(i,j), data.s_A,...
            data.s_V, a_A, b_A, sigma_AV_A, sigma_AV_V, epsilon, CI_prepost, mu_P,...
            lapse_unity_i, Dij_unity, Dij_locResp, data.numUnityTrialsPerLoc,...
            data.sigma_r, model);
    end
    
    %----------------------------------------------------------------------
    %------------FOR THE LEARNING PHASE (THE LAST THIRD TRIALS)------------
    %----------------------------------------------------------------------
    eval(['sigma_AV_A = sigma_AV_A_', model.cond{i},';']);
    eval(['sigma_AV_V = sigma_AV_V_', model.cond{i},';']);
    eval(['s_A = data.',model.cond{i},'_Alocs;']);
    eval(['s_V = data.',model.cond{i},'_Vlocs;']);
    eval(['D_unity_learning = data.',model.cond{i},'_unity;']);
    eval(['D_locResp_learning = data.',model.cond{i},'_locResp;']);
    eval(['locMod = data.',model.cond{i},'_locModality;']);
    CI_learning = calcCI(sigma_AV_A,sigma_AV_V,sigma_P);
    nLL_learning(i) = calculateNLL_learning(pC1(i,end), s_A, s_V, a_A, b_A, ...
        sigma_AV_A, sigma_AV_V, epsilon, CI_learning, mu_P, lapse_unity_i, ...
        D_unity_learning, D_locResp_learning, locMod, length(data.learning_sT),...
        data.sigma_r, model);
end

%sum the nLL from all tasks together
nLL = nLL_unimodal + nLL_matching + sum(nLL_bimodal(:)) + sum(nLL_learning);
...
function [nLL_bimodal,R] = calculateNLL_bimodal(pC1, s_A, s_V, a_A, b_A, ...
    sigma_AV_A, sigma_AV_V, epsilon, CI, mu_P, lapse, D_unity, D_locResp, ...
    N, sigma_r, model)
nLL_bimodal = 0;
...
for p = 1:length(s_A_prime)   %for each AV pair with s_A' = s_A_prime(p)
    ...
    for q = 1:length(s_V_prime) %for each AV pair with s_V' = s_V_prime(q)
        ...
        %first 10 trials are A loc resps, last 10 trials are V loc resps
        locResp_A_V = [squeeze(D_locResp(p,q,1,:))', squeeze(D_locResp(p,q,2,:))']; 
        if strcmp(model.strategy_MAP,'MA') %Model averaging
            for k = 1:N %N=20
                idx = fix((k-1)/(N/2))+1; 
                %idx = 1 for the first 10 trials; = 2 for the last 10 trials
                p_r_given_MAP = norm_dst(locResp_A_V(k),squeeze(MAP_MA(idx,:,:)),...
                                sigma_r,1e-20);
                ...
                nLL_bimodal = nLL_bimodal - log(sum(sum(p_r_given_MAP.*...
                                p_I_given_m.*p_mAmV_given_sAsV)));
            end
        else %Model selection
            for k = 1:N
                idx = fix((k-1)/(N/2))+1;
                if D_unity(p,q,k)==1 %same source
                    p_I_given_m   = P_C1_resp; 
                    p_r_given_MAP = norm_dst(locResp_A_V(k),shat_C1,sigma_r,1e-20);
                else %separate sources
                    p_I_given_m   = 1 - P_C1_resp; 
                    p_r_given_MAP = norm_dst(locResp_A_V(k),squeeze(shat_C2(idx,:,:)),...
                                    sigma_r,1e-20);
                end
                nLL_bimodal = nLL_bimodal - log(sum(sum(p_r_given_MAP.*...
                    p_I_given_m.*p_mAmV_given_sAsV)));
...
function nLL_learning = calculateNLL_learning(pC1, s_A, s_V, a_A, b_A, ...
    sigma_AV_A, sigma_AV_V, epsilon, CI, mu_P, lapse, D_unity, D_locResp, ...
    locModality, N, sigma_r, model)
nLL_learning = 0;
s_V_prime  = s_V;
for p = 1:N
    s_A_prime   = s_A.*a_A + b_A; %the mean of biased auditory measurements
    x1_grid     = linspace(s_A_prime(p) - model.numSD*sigma_AV_A,...
                           ...
                           model.numBins_A);
    mDist_AV_A = norm_dst(x1_grid, s_A_prime(p), sigma_AV_A, 0);
    x2_grid    = linspace(s_V_prime(p) - model.numSD*sigma_AV_V,...
                          s_V_prime(p) + model.numSD*sigma_AV_V,...
                          model.numBins_V);
    mDist_AV_V = norm_dst(x2_grid, s_V_prime(p), sigma_AV_V, 0);
    %use outer product to get the bi-variate distribution
    p_mAmV_given_sAsV = mDist_AV_V'.*mDist_AV_A; 
    p_mAmV_given_sAsV = p_mAmV_given_sAsV./(sum(p_mAmV_given_sAsV(:)));
    [GRID_X1, GRID_X2]= meshgrid(x1_grid, x2_grid); 

    %calculate the likelihood and the posterior
    [Post_C1, Post_C2, ~, ~]= calculatePostC1C2(GRID_X1, GRID_X2, CI, mu_P, pC1); 

    %calculate the shat_{C=1}, shat_{C=2} and MAP estimates assuming MA
    shat_C1           = (GRID_X1./CI.J_A + GRID_X2./CI.J_V + ...
                        mu_P./CI.J_P)./CI.constC1_shat;
    [shat_C2, MAP_MA] = deal(NaN(length(model.modality), model.numBins_V,...
                        model.numBins_A));
    ...
    %--------------------- Localization responses ---------------------
    if locModality(p) == 1; idx = 1; else; idx = 2; end
    if strcmp(model.strategy_MAP,'MA') %Model averaging
        %idx = 1 for localizing A trials; = 2 for localizing V trials
        p_r_given_MAP = norm_dst(D_locResp(p),squeeze(MAP_MA(idx,:,:)),...
                        sigma_r,1e-20);
        if ~isnan(D_unity(p))
            if D_unity(p) == 1; p_I_given_m = P_C1_resp; 
            else; p_I_given_m = 1 - P_C1_resp; end
            nLL_learning = nLL_learning - log(sum(sum(p_r_given_MAP.*...
                            p_I_given_m.*p_mAmV_given_sAsV)));
        else
            nLL_learning = nLL_learning - log(sum(sum(p_r_given_MAP.*...
                            p_mAmV_given_sAsV)));        
        ...
        if ~isnan(D_unity(p)) %subjects were asked to report unity judgment
            if D_unity(p)==1 %same source
                p_I_given_m   = P_C1_resp; 
                p_r_given_MAP = norm_dst(D_locResp(p),shat_C1,sigma_r,1e-20);
            else %separate sources
                p_I_given_m   = 1 - P_C1_resp; 
                p_r_given_MAP = norm_dst(D_locResp(p),squeeze(shat_C2(idx,:,:)),...
                                sigma_r,1e-20);
            end
            nLL_learning = nLL_learning - log(sum(sum(p_r_given_MAP.*...
                p_I_given_m.*p_mAmV_given_sAsV)));
        else %subjects were not asked to report unity judgment
            p_r_given_MAP_C1 = norm_dst(D_locResp(p),shat_C1,sigma_r,1e-20);
            p_r_given_MAP_C2 = norm_dst(D_locResp(p),squeeze(shat_C2(idx,:,:)),...
                                sigma_r,1e-20);
            nLL_learning = nLL_learning - log(sum(sum((p_r_given_MAP_C1.*...
                P_C1_resp + p_r_given_MAP_C2.*(1 - P_C1_resp)).*p_mAmV_given_sAsV)));
        end
    end
end

function const = calcCI(sigma_AV_A, sigma_AV_V, sigma_P)
%constants for fitting the unity judgment
const.J_A            = sigma_AV_A^2;
const.J_V            = sigma_AV_V^2;
const.J_P            = sigma_P^2;
const.constC1        = const.J_A*const.J_V + const.J_A*const.J_P + const.J_V*const.J_P;
const.constC1_shat   = 1/const.J_A  + 1/const.J_V + 1/const.J_P;
const.constC2_1      = const.J_A + const.J_P;
const.constC2_1_shat = 1/const.J_A + 1/const.J_P;
const.constC2_2      = const.J_V + const.J_P;
const.constC2_2_shat = 1/const.J_V + 1/const.J_P;   
