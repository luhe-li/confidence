...
clear all; close all; clc; parpool(2); %2 cores, 3 hours
subjN = 15;
subjI = 'AD';
...
%% load the last 1/3 data from the learning phase
% addpath(genpath(['/Users/hff/Desktop/NYU/Project2/Experiment code/',...
...
%p(5)-p(7)  : sigma_AV_A_prepost, sigma_AV_A_cong, sigma_AV_A_incong
%p(8)-p(10) : sigma_AV_V_prepost, sigma_AV_V_cong, sigma_AV_V_incong
%p(11)      : episolon (only applicable for some strategy)
%p(12)-p(13): pC1_pre_cong, pC1_post_cong, pCommon for the pre- and the
%               post-association phase
%p(14)-p(15): pC1_pre_incong, pC1_post_incong, pCommon for the pre- and the
%               post-dissociation phase
%p(15)-p(18): lapse rate for the matching task and the unity judgments for
%               the congruent and incongruent condition
%--------------------------------------------------------------------------
nLogL     = @(p) nLL_overall_wLearningPhase_diffSigma(p(1),p(2),p(3),p(4),...
                    p(5),p(6),p(7),p(8),p(9),p(10),p(11),p(12),p(13),p(14),...
                    p(15),p(16),p(17),p(18),data,model);
model.lb  = [0.5,-20, 1e-1, 1e-2,1e-1,1e-1,1e-1, 1e-2, 1e-2, 1e-2, 0.1, 1e-2, 1e-2,1e-2, 1e-2, 1e-4, 1e-4, 1e-4];
model.ub  = [3.5, 10,   15,    5,  15,  15,  25,   10,   10,   15,  30, 0.99, 0.99,0.99, 0.99, 0.06, 0.06, 0.06];
model.plb = [1.5, -9,    1,  0.1,   5,   5,   5,    2,    2,    2,   5,  0.1,  0.1, 0.1,  0.1, 1e-2, 1e-2, 1e-2];
model.pub = [  2,  0,    5,    2,  10,  10,  10,    5,    5,    5,  10,  0.5,  0.5, 0.5,  0.5, 0.03, 0.03, 0.03];
...
save(['ModelFitting_updatePrior_fullModel_strategyMAP_', model.strategy_MAP, ...
    '_strategyUnity_', model.strategy_unity, '_wLearningPhase_diffSigma_sub',...
