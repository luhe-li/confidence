...
Summary_VE               = {subjN, meanVE, sdLocR_pooled, seLocR_pooled,...
                            meanVE_abs_spatialD, sdLocR_abs_spatialD,...
                            seLocR_abs_spatialD, mean_pC1_lenD, SD_pC1_lenD,...
                            meanPC1_abs_spatialD, SD_pC1_abs_spatialD};
if bool_save(1) == 1
...
%% export table to excel file (percent of reporting C=1, VE, invVE)
[prct_mat, VE_Ashift, VE_Vshift] = deal([]);
for i = 1:lenC
    for j = 1:lenP
        prct_mat  = [prct_mat; squeeze(mean_pC1_lenD(i,j,:))];
        VE_Ashift = [VE_Ashift; squeeze(meanVE(i,j,1,:))];
        VE_Vshift = [VE_Vshift; squeeze(meanVE(i,j,2,:))];
    end
end

[sI_cell, cond_cell, phase_cell] = deal(cell(lenC*lenP*lenD,1));
spatialD_mat  = repmat(AV_discrepancy',[lenC*lenP,1]);
sI_cell(:)    = {subjI};
cond_cell(:)  = {cond{1}}; cond_cell((lenD*2+1):end) = {cond{2}};
phase_cell(:) = {phase{1}}; phase_cell([(lenD+1):lenD*2,(lenD*3+1):lenD*4])={phase{2}};
Table_prctC1   = table(prct_mat, cond_cell, phase_cell,spatialD_mat, sI_cell,...
                    'VariableNames', {'PercentC1', 'Condition','Phase',...
                    'SpatialD','SubjI'});
Table_VE_Ashift = table(VE_Ashift, cond_cell, phase_cell,spatialD_mat, sI_cell,...
                    'VariableNames', {'Ashift_VE', 'Condition','Phase',...
                    'SpatialD','SubjI'});
Table_VE_Vshift = table(VE_Vshift, cond_cell, phase_cell,spatialD_mat, sI_cell,...
                    'VariableNames', {'Vshift_invVE', 'Condition','Phase',...
                    'SpatialD','SubjI'});
if bool_save(2) == 1
    writetable(Table_prctC1, ['prctC1_',subjI,'.xlsx'],'Sheet','MyNewSheet',...
        'WriteVariableNames',true);
    writetable(Table_VE_Ashift, ['VE_Ashift_',subjI,'.xlsx'],'Sheet',...
        'MyNewSheet', 'WriteVariableNames',true);
    writetable(Table_VE_Vshift, ['VE_Vshift_',subjI,'.xlsx'],'Sheet',...
        'MyNewSheet', 'WriteVariableNames',true);    
end
...
Table_unity   = table(unity_mat, cond_cell, phase_cell,spatialD_mat,...
    abs(spatialD_mat), sI_cell, 'VariableNames', {'UnityJudgment',...
    'Condition','Phase','SpatialD','SpatialD_abs','SubjI'});
...
%(unimodal localization responses - mean unimodal localization responses)

%create a cell array that stores the response modality of each trial
%numT_AV = [20, 40, 60, 80, 60, 40, 20];
%'A' * 10 times, 'V' * 10 times, 
%'A' * 20 times, 'V' * 20 times,
%'A' * 30 times, 'V' * 30 times,
%...
modality_cell(:) = {modality{1}}; 
get_idx_mod_A    = @(vec, idx) sum(vec(1:idx)) + vec(idx+1)/2 + 1;
idx_mod_A        = arrayfun(@(idx) get_idx_mod_A([0, numT_AV], idx), 1:lenD);
idx_mod_A_all    = [idx_mod_A, idx_mod_A + sum(numT_AV), idx_mod_A + 2*sum(numT_AV),...
                    idx_mod_A + 3*sum(numT_AV)];
idx_mod_AV_all   = [cumsum(numT_AV), cumsum(numT_AV) + sum(numT_AV), ...
                    cumsum(numT_AV) + 2*sum(numT_AV), cumsum(numT_AV) + 3*sum(numT_AV)];
for i = 1:length(idx_mod_A_all)
    modality_cell(idx_mod_A_all(i):idx_mod_AV_all(i)) = {modality{2}};
end

%VE = bimodal localization responses - mean unimodal localization response
%locR_mat_corr represents VE with corrected sign
%locR_mat = -1.5 when the response modality is 'A' and spatialD = V - A = -24,
%           then locR_mat_corr = 1.5
%locR_mat = -0.5 when the response modality is 'V' and spatialD = V - A = 24, 
%           then locR_mat_corr = 0.5
locR_mat_corr   = locR_mat;
idx_A_sign_corr = (spatialD_mat < 0) & (cell2mat(modality_cell) == 'A');
locR_mat_corr(idx_A_sign_corr) = -1.*locR_mat_corr(idx_A_sign_corr);
idx_V_sign_corr = (spatialD_mat > 0) & (cell2mat(modality_cell) == 'V');
locR_mat_corr(idx_V_sign_corr) = -1.*locR_mat_corr(idx_V_sign_corr);

Table_locR  = table(locR_mat, locR_mat_corr, cond_cell, phase_cell,...
    modality_cell, spatialD_mat, abs(spatialD_mat), sI_cell,...
    'VariableNames', {'LocResp', 'LocResp_crr','Condition','Phase',...
    'Modality','SpatialD','SpatialD_abs', 'SubjI'});
