function varargout = guitest_update_pC1_maskingEffects(varargin)
% GUITEST_UPDATE_PC1_MASKINGEFFECTS MATLAB code for guitest_update_pC1_maskingEffects.fig
%      GUITEST_UPDATE_PC1_MASKINGEFFECTS, by itself, creates a new GUITEST_UPDATE_PC1_MASKINGEFFECTS or raises the existing
%      singleton*.
%
%      H = GUITEST_UPDATE_PC1_MASKINGEFFECTS returns the handle to a new GUITEST_UPDATE_PC1_MASKINGEFFECTS or the handle to
%      the existing singleton*.
%
%      GUITEST_UPDATE_PC1_MASKINGEFFECTS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUITEST_UPDATE_PC1_MASKINGEFFECTS.M with the given input arguments.
%
%      GUITEST_UPDATE_PC1_MASKINGEFFECTS('Property','Value',...) creates a new GUITEST_UPDATE_PC1_MASKINGEFFECTS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guitest_update_pC1_maskingEffects_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guitest_update_pC1_maskingEffects_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guitest_update_pC1_maskingEffects

% Last Modified by GUIDE v2.5 17-Apr-2021 00:34:30

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @guitest_update_pC1_maskingEffects_OpeningFcn, ...
                   'gui_OutputFcn',  @guitest_update_pC1_maskingEffects_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before guitest_update_pC1_maskingEffects is made visible.
function guitest_update_pC1_maskingEffects_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guitest_update_pC1_maskingEffects (see VARARGIN)

% Choose default command line output for guitest_update_pC1_maskingEffects
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes guitest_update_pC1_maskingEffects wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guitest_update_pC1_maskingEffects_OutputFcn(hObject, eventdata, handles) 
...
% save('dict_saveP_update_pC1_maskingEffects.mat', 'cell_savedP');
...
addpath(genpath(['/Users/Guinevere/Desktop/NYU/Project2/Experiment code/',...
                'ModelFitting/Fits']));
...
param.sigma_spatial_AV_A = estimatedP(5);
...
elseif get(handles.radiobutton_saved,'Value') == 1
    ...
    %----------------------------------------------------------------------
    C = load('dict_saveP_update_pC1_maskingEffects.mat', 'cell_savedP');
...
%--------------------------------------------------------------------------
set_name = {'gamma_AV_A_lb_ub_congruent','gamma_AV_V_lb_ub_congruent',...
    'gamma_AV_A_lb_ub_incongruent','gamma_AV_V_lb_ub_incongruent','tol_pC1',...
...
if get(handles.radiobutton_saved,'Value') == 1
    ...
    for i = 1:numBins %A
        ...
        for j = 1:numBins %V
            for k = 1:D.numSims
                for l = 1:numCond
                    eval(['param.gamma_AV_A = gamma_AV_A_', Cond{l},'(i);']);
                    eval(['param.gamma_AV_V = gamma_AV_V_', Cond{l},'(j);']);
                    eval(['param.alpha = param.alpha_pC1_', Cond{l}, ';']);
                    param.pCommon = emp_pCommon_pre(l); 
                    [propC1(l,i,j,k), pCommon_end(l,i,j,k)] = ...
                        simulateLearningPhase_update_pC1_maskingEffects(...
...
for i = 1:numCond %2 conditions
    for j = 1:length(d)
        try
            ...
            imagesc(eval(['gamma_AV_V_',Cond{i}]), eval(['gamma_AV_A_',Cond{i}]),...
                squeeze(d{j}(i,:,:))); 
            c = colorbar; set(c,'XTick',cticks); caxis([cticks(1),cticks(end)]); 
            eval(['colormap(handles.axes',num2str((i-1)*numCond+j),',cMap)']); hold on;
            drawLines(eval(['gamma_AV_V_',Cond{i}]), eval(['gamma_AV_A_',Cond{i}]), ...
                squeeze(d_bool{j}(i,:,:))); hold off;
            xlabel('\gamma_{AV,V}'); ylabel('\gamma_{AV,A}');
            eval(['yticks(round(gamma_AV_V_', Cond{i},'(1:3:end),2));']);
            eval(['yticks(round(gamma_AV_A_', Cond{i},'(1:3:end),2));']);
        catch
            text(3,3,'ERROR');
        end    
        if j == 1
...
symbs2 = {'\gamma_{AV,A,cong} (lb,ub):', '\gamma_{AV,V,cong} (lb,ub):',...
    '\gamma_{AV,A,incong} (lb,ub):','\gamma_{AV,V,incong} (lb,ub):',...
    'tol_1:', 'tol_2:','#grids:'};
...
function text_gamma_AV_A_lb_ub_congruent_Callback(hObject, eventdata, handles)

% --- Executes during object creation, after setting all properties.
function text_gamma_AV_A_lb_ub_congruent_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), ...
    get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
function text_gamma_AV_V_lb_ub_congruent_Callback(hObject, eventdata, handles)

% --- Executes during object creation, after setting all properties.
function text_gamma_AV_V_lb_ub_congruent_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), ...
    get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function text_gamma_AV_A_lb_ub_incongruent_Callback(hObject, eventdata, handles)

% --- Executes during object creation, after setting all properties.
function text_gamma_AV_A_lb_ub_incongruent_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'),... 
    get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function text_gamma_AV_V_lb_ub_incongruent_Callback(hObject, eventdata, handles)

% --- Executes during object creation, after setting all properties.
function text_gamma_AV_V_lb_ub_incongruent_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), ...
    get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
