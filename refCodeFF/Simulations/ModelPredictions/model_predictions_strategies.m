...
modelTypes           = 'full';
strat_unity          = {'MAP','posteriorC1','measurements'};
...
lenS = length(s_A);
%initialize a matrix
[uniJdg_perc_true, uniJdg_perc_sim] = deal(NaN(length(strat_unity), lenC, lenP, lenS, lenS));
uniJdg_binary_sim = NaN(length(strat_unity), lenC, lenP, lenS, lenS, nT_perPair);
[pC1_resp_lenD_true, uniJdg_perc_sim_lenD,uniJdg_perc_sim_SD_lenD] = ...
    deal(NaN(length(strat_unity), lenC, lenP, lenD));
for m = 1:length(strat_unity)
    model.strategy_unity = strat_unity{m};
    for i = 1:lenC
        for j = 1:lenP
            %get model prediction
            [~,R] = get_mPred(pC1(i,j),epsilon,s_A, s_V, a_A, b_A, sigma_AV_A,...
...
%         numT_AV, strat_unity{m},1);
    plt_uniJdg_perc_sim(spatialD, squeeze(pC1_resp_lenD_true(m,:,:,:)),[],...
        [],[],numT_AV, strat_unity{m},1);
end

%% fit models (with different number of pCommons) to the simulated data
% %first initialize matrices)
% fit_pCommon         = NaN(length(modelTypes), lenC, lenP);
% pC1_resp_mPred      = NaN(length(modelTypes), lenC, lenP, lenS, lenS);
% pC1_resp_lenD_mPred = NaN(length(modelTypes), lenC, lenP, lenD); 
% 
% for m = 1:length(modelTypes)
%     %fit the model to the similated data
%     fit_pCommon(m,:,:) = fit_uniJdg_sim(uniJdg_binary_sim, s_A, s_V,...
%         a_A, b_A, sigma_AV_A, sigma_AV_V, epsilon, CI, mu_P, lapse, model,...
%         modelTypes{m});
% 
%     %get model predictions given the best-fit pCommon
%     for i = 1:lenC
%         for j = 1:lenP
%             [~,R] = get_mPred(fit_pCommon(m,i,j),epsilon, s_A, s_V, a_A, b_A,...
%                 sigma_AV_A, sigma_AV_V, CI, mu_P, lapse, model, []);
%             pC1_resp_mPred(m,i,j,:,:) = R.pC1_given_sAsV;
%         end
%     end
%     %reshape
%     [~, pC1_resp_lenD_mPred(m,:,:,:), ~, ~] = reshapeResp(squeeze(...
%         pC1_resp_mPred(m,:,:,:,:)), nT_perPair);
% end
% 
% for m = 1:length(modelTypes)
%     %plot the simulated data with model 
%     %pC1_resp = pC1_resp_lenD_true;
%     if m == 1; pC1_resp = pC1_resp_lenD_true; else; pC1_resp = [];end
%     plt_uniJdg_perc_sim(spatialD, pC1_resp, squeeze(pC1_resp_lenD_mPred(m,:,:,:)),...
%         uniJdg_perc_sim_lenD, uniJdg_perc_sim_SD_lenD, numT_AV, modelTypes{m},0);
% end
