...
sesNum            = order_dict(subj_idx,2); %congruent condition
...
if strcmp(model.M_pC1, 'fullModel'); pC1_idx = [10,11];
elseif strcmp(model.M_pC1, 'samePC1pre'); pC1_idx = [8, 10];
...
param.mu_P        = 0;
param.sigma_P     = 100;
param.sigma_AV_A  = estimatedP(5); %deg
param.sigma_AV_V  = estimatedP(6);
param.a_A         = estimatedP(1);
param.b_A         = estimatedP(2);
param.pCommon     = estimatedP(pC1_idx(1));
param.c_unityJdg  = estimatedP(7);
alpha_pC1         = logspace(-2, -0.6, 5); %YZ: logspace(-2, -0.6, 5); 
...
sigma_A_learning = sqrt(data_learning.locVar{1}(subj_idx,2)^2 - sigma_r^2);
sigma_V_learning = sqrt(data_learning.locVar{2}(subj_idx,2)^2 - sigma_r^2);
if imag(sigma_A_learning)~=0; sigma_A_learning = 1e-2;end
if imag(sigma_V_learning)~=0; sigma_V_learning = 1e-2;end

sigma_A_learning_vec = linspace(sigma_A_learning*0.9, sigma_A_learning*1.2, 20); %YZ: [0.9, 1.2], BB: [0.5, 1.2]
sigma_V_learning_vec = linspace(sigma_V_learning*0.9, sigma_V_learning*1.2, 20);

%% stimulus pairs presented during the experiments
addpath(genpath(['/Users/hff/Desktop/NYU/Project2/Adaptation v2/Data/',subjI]));
F = load(['Adaptation_incongruent_sub', num2str(subjN),'_session',...
    num2str(sesNum),'.mat'], 'Adaptation_incongruent_data');
D.AVpairs(1,:)        = F.Adaptation_incongruent_data{4}.arrangedLocs_deg; %A loc
D.AVpairs(2,:)        = F.Adaptation_incongruent_data{3}.arrangedLocs_deg; %V loc
D.AVpairs(3,:)        = -F.Adaptation_incongruent_data{3}.timing_relative./2; %A time relative
D.AVpairs(4,:)        = F.Adaptation_incongruent_data{3}.timing_relative./2; %V time relative
D.bool_unity          = ~isnan(F.Adaptation_incongruent_data{end}.unity);
...
for i = 1:length(alpha_pC1)
    ...
    xticks(round(sigma_V_learning_vec(1:6:end),1));
...
%saveas(gcf, ['Simtest_incong_', subjI], 'pdf'); 
                
%%
figure
max_v2 = max(simReportingC1(:)); min_v2 = min(simReportingC1(:));
cmap_cus2 = customize_colormap(max_v2, min_v2, empReportingC1, ...
    [167,242,78]./255, [0, 139,0]./255, ones(1,3));colormap(cmap_cus2); 
for i = 1:length(alpha_pC1)
    subplot(1,length(alpha_pC1),i)
    imagesc('XData',sigma_V_learning_vec,'YData',sigma_A_learning_vec,...
        'CData',squeeze(simReportingC1(:,:,i))); 
    caxis([min_v2, max_v2]);
    xlim([sigma_V_learning_vec(1), sigma_V_learning_vec(end)]);
    ylim([sigma_A_learning_vec(1), sigma_A_learning_vec(end)]);
    if i == 1
