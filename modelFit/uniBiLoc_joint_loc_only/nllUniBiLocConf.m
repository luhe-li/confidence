...
switch model.mode
    ...
    case 'initiate'
        ...
        out.paraID                   = {'aA','bA','\sigma_{V1,uni}','\sigma_{A,uni}','\sigma_{V2,uni}','\sigma_{V1,bi}','\sigma_{A,bi}','\sigma_{V2,bi}','\sigma_{P}','p_{common}'};
    ...
    case {'optimize','predict'}
        ...
        elseif strcmp(model.mode, 'predict')
            ...
            [out.uni_loc] = simUni(...
                aA, bA, sigAuni, sigV1uni, sigV2uni, muP, sigP,  fixP);

            [bi_loc] = deal(NaN(numel(bi_sA), numel(bi_sV), numel(model.modality), numel(sigVs), model.bi_nrep));
            ...
            for aa = 1:numel(bi_sA)
                for vv = 1:numel(bi_sV)
                    for rr = 1:numel(sigVs)
                        ...
                        [bi_loc(aa,vv,:,rr,:)] = simAllModels(...
                            aA, bA, sigA, sigVs(rr), muP, sigP, pCommon, fixP);
    ...
    % function nLL_uni_conf = calculateNLL_uniConf(m, v, c1, c2, c3,  data)
    % 
