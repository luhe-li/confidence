...
switch model.mode
    ...
    case 'initiate'
        ...
        out.paraID                   = {'aA','bA','\sigma_{V1}','\sigma_{A} - \sigma_{V1}','\sigma_{V2} - \sigma_{V1}','\sigma_{P}','p_{common}','\sig_{c}','criterion_{A}','criterion_{V}'};
        ...
        paraH.delta_sigA             = [1e-1,     2]; % degree
        paraH.delta_sigV2            = [1e-1,     2]; % degree
        paraH.sigP                   = [  10,    20]; % degrees
        paraH.pC1                    = [1e-3,1-1e-3]; % weight
        paraH.sigC                   = [ 0.3,     1]; % measurement noise of confidence
        paraH.cA                     = [ 0.1,    10]; % 
        paraH.cV                     = [ 0.1,    10]; % 
...
%             var(1:2,:,:) = repmat(1/CI.constC1_shat, [2, size(Post_C1)]);
...
%             var(2,:,:) = 1/CI.constC2_2_shat;
        ...
        % and visual criteria.
        temp_p_conf(1,:,:) = logncdf(cA, log(var(1,:,:)), sigC);
        temp_p_conf(2,:,:) = logncdf(cV, log(var(2,:,:)), sigC);
