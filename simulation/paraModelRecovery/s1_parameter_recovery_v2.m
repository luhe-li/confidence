...
if exist(flnm,'file')
    ...
    GT.delta_sigA         = [   1,     3]; % degree
    GT.delta_sigV2        = [   2,     4]; % degree
    GT.sigP               = [  10,    15]; % degree
    GT.pC1                = [ 0.3,   0.7]; % weight
    GT.Acriterion1         = [   0.6,     0.65];  
    GT.Acriterion2         = [   0.6,     0.65];  
    GT.Acriterion3         = [   0.6,     0.65]; 
    GT.Vcriterion1        = [   0.6,     0.65]; 
    GT.Vcriterion2        = [   0.6,     0.65]; 
    GT.Vcriterion3        = [   0.6,     0.65]; 

    fn                    = fieldnames(GT);
    temp_num_para         = numel(fn);
    num_para              = temp_num_para - 5;
    [lb, ub]              = deal(NaN(1,  temp_num_para));
    for k                 = 1:numel(fn)
        lb(:,k)               = GT.(fn{k})(1);
        ub(:,k)               = GT.(fn{k})(2);
    end
    ...
    samples               = lb + (ub - lb) .* rand(num_sample, temp_num_para);
    ...
    for i                 = 1:num_sample
        ...
        bA                    = samples(i, 2);
        sigV1                 = samples(i, 3);
        sigA                  = samples(i, 4) + sigV1;
        sigV2                 = samples(i, 5) + sigV1;
        sigVs                 = [sigV1, sigV2];
        sigP                  = samples(i, 6);
        pCommon               = samples(i, 7);
        Acriterion            = [samples(i, 8),samples(i, 9),samples(i,10)];% a criterion for 3 models
        Vcriterion            = [samples(i, 11),samples(i, 12),samples(i,13)]; % v criterion for 3 models
        ...
        for j                 = 1:num_s
            for v                 = 1:numel(sigVs)
                [loc(j,:,:,v,:), conf(j,:,:,v,:), variance(j,:,:,v,:)] = sim_loc_pconf(num_rep, sAV(1,j),...
                    sAV(2,j), aA, bA, sigA, sigVs(v), sigP, pCommon, Acriterion, Vcriterion, fixP);
            end
        end
...
for dd                = 1:num_model
    ...
    for jj                = 1:num_para
        ...
        scatter(samples(:,jj)', saveModel{d}.bestP(dd,:,jj), 'k','filled');
        xlabel('Ground Truth');
        ylabel('Predicted');
        ...
        % plot identity line
        minVal = min([samples(:,jj)', saveModel{d}.bestP(dd,:,jj)]);
        maxVal = max([samples(:,jj)', saveModel{d}.bestP(dd,:,jj)]);
