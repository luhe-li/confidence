...
for xx = 1:numel(fixP.px_axis) % loop by pixel, not pixel grid
    [radius(xx,:), gain(xx,:)] = eGain(post_2d, ones(size(post_2d,1),1).* xx, fixP.maxScore, fixP.minScore, fixP.elbow, numel(fixP.px_axis));
end
[~, idx_opt] = max(gain, 2);
opt_est = fixP.px_axis(idx_opt);
opt_radius = radius(idx_opt);

% motor noise to location estimation
bi_loc = randn(size(opt_radius)).*sigMotor + opt_est;
bi_loc = reshape(bi_loc, [n_sA, n_sA, 2, 2, bi_loc]) - 512;
