function [nLL, R1, R2] = nLL_NoPlasticity(a_A, b_A, sigma_P, sigma_A, ...
    sigma_V, sigma_AV_A, sigma_AV_V, epsilon, pC1, lapse_matching, ...
...
s_V_prime_uni      = a_V.*data.unimodal(1,:,2) + b_V;
...
%               2nd dimension, trial number
nLL_matching    = calculateNLL_matching(a_A, b_A, a_V, b_V, c_A, c_V, f_A,...
    f_V, data.s_V, sigma_shat_A, sigma_shat_V, lapse_matching, data.matching);
...
for i = 1:length(model.cond) 
    ...
    for j = 1:length(model.phase) 
        ...
        [nLL_bimodal(i,j),R2{i,j}] = calculateNLL_bimodal(pC1, data.s_A,...
            data.s_V, a_A, b_A, a_V, b_V, sigma_AV_A, sigma_AV_V, epsilon,...
            CI, mu_P, lapse_unity_i, Dij_unity, Dij_locResp, ...
            data.numUnityTrialsPerLoc, data.sigma_r, model);
    end
end
...
function nLL_matching = calculateNLL_matching(a_A, b_A, a_V, b_V, c_A,...
    c_V, f_A, f_V, s_V, sigma_shat_A, sigma_shat_V, lapse, D)
nLL_matching = 0;
...
for j = 1:length(D) %for each standard visual stimulus (there are 4)
    mu_diff = c_A.*(a_A.*D{j}(1,:) + b_A) + f_A - (c_V*(a_V.*s_V(j)+b_V) + f_V); 
...
function [nLL_bimodal,R] = calculateNLL_bimodal(pC1, s_A, s_V, a_A, b_A, ...
    a_V, b_V, sigma_AV_A, sigma_AV_V, epsilon, CI, mu_P, lapse, D_unity, ...
    D_locResp, N, sigma_r, model)
nLL_bimodal = 0;
...
for p = 1:length(s_A_prime)   %for each AV pair with s_A' = s_A_prime(p)
    ...
    for q = 1:length(s_V_prime) %for each AV pair with s_V' = s_V_prime(q)
        ...
        if strcmp(model.strategy_locResp,'MA') %Model averaging
            for k = 1:N
                idx = fix((k-1)/(N/2))+1; 
        ...
        if strcmp(model.strategy_locResp,'MA')
            R.MAP(p,q,:,:,:) = MAP_MA;
        elseif strcmp(model.strategy_locResp,'MS') && strcmp(model.strategy_unity,'posteriorC1')
